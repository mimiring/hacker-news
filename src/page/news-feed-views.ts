...
export default class NewsFeedView extends View {
  ...
  constructor(containerId: string) {
    ...
    if (this.feeds.length === 0) {
      ...
      this.makeFeed();
      window.store.lastPage = Math.ceil(this.feeds.length / 10);
    }

    if (
      window.store.currentPage < 1 ||
      window.store.currentPage > window.store.lastPage
    ) {
  ...
  render(): void {
    ...
    this.setTemplateData(
      ...
      String(
        window.store.currentPage + 1 > window.store.lastPage
          ? window.store.lastPage
          : window.store.currentPage + 1
      )
    );
